...
const ReplicationRequestSchema = Schema.Struct({
  tableId: Schema.Literals([208, 347]),
  institutionCode: Schema.String.pipe(Schema.check(Schema.isPattern(/^\d{4}$/))),
  year: Schema.Int.pipe(Schema.check(Schema.isBetween({ minimum: 2000, maximum: 2200 }))),
  semester: Schema.Literals([1, 3]),
  programmeCode: Schema.String.pipe(Schema.check(Schema.isPattern(/^[A-ZÆØÅ0-9-]{2,30}$/))),
});
...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    if (
      ...
      const decoded = Schema.decodeUnknownResult(ReplicationRequestSchema)(input);
      if (Result.isFailure(decoded))
        return jsonError(400, 'Replication request failed boundary validation.');
      try {
        const result = await replicateDbhEvidence(decoded.success, dependencies(env));
        return Response.json({
          source: 'dbh',
          tableId: decoded.success.tableId,
