...
const decodeCourseSearchItemSchema = Schema.decodeUnknownSync(CourseSearchItemSchema);
const decodeCourseGradeSummarySchema = Schema.decodeUnknownSync(CourseGradeSummarySchema);
const decodeCourseDecisionSignalsSchema = Schema.decodeUnknownSync(CourseDecisionSignalsSchema);

export type EvidenceBearingModel =
  | CourseInsight
  | CourseSearchItem
  | CourseGradeSummary
  | CourseDecisionSignals;

export type EvidenceReferenceFinding = {
  readonly path: string;
  readonly evidenceId: string;
};

export const validateEvidenceReferences = (
  model: EvidenceBearingModel,
): ReadonlyArray<EvidenceReferenceFinding> => {
  const availableIds = new Set(model.evidence.map((item) => item.id));
  const missing: Array<EvidenceReferenceFinding> = [];
...
const assertEvidenceReferences = (model: EvidenceBearingModel): void => {
  const missing = validateEvidenceReferences(model);
  if (missing.length === 0) return;
  throw new TypeError(
    missing.map(({ path, evidenceId }) => `${path}: unknown evidence ID ${evidenceId}`).join('\n'),
  );
};

const decodeEvidenceBearing = <Model extends EvidenceBearingModel>(
  decode: (input: unknown) => Model,
  input: unknown,
): Model => {
  const model = decode(input);
  assertEvidenceReferences(model);
  return model;
};
...
export const decodeCourseSearchItem = (input: unknown): CourseSearchItem =>
  decodeEvidenceBearing(decodeCourseSearchItemSchema, input);

export const decodeCourseGradeSummary = (input: unknown): CourseGradeSummary =>
  decodeEvidenceBearing(decodeCourseGradeSummarySchema, input);

export const decodeCourseInsight = (input: unknown): CourseInsight => {
  const insight = decodeEvidenceBearing(decodeCourseInsightSchema, input);
...
export const decodeCourseDecisionSignals = (input: unknown): CourseDecisionSignals => {
  const signals = decodeEvidenceBearing(decodeCourseDecisionSignalsSchema, input);
